...
class KeyInfoView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_superuser:
            ...
            self.context['user_list'] = list(User.objects.values(*data_cols_args).order_by('username'))
...
class UserAPI(View):
    ...
    def get(self, request):
        try:
            ...
            val = list(models.Keyinfo.objects.values(
    ...
    def post(self, request):
        ...
        try:
            ...
            insert_audit_log(request.user.id, request, category, '사용자', audit_log, result)
        ...
        try:
            if key:
                ...
                if models.Keyinfo.objects.filter(user_id=request.user.id, key=key):
                    keyinfo_data = models.Keyinfo.objects.get(user_id=request.user.id, key=key)

                    if keyinfo_data.description != description:
                        # 감사 로그 > 내용 추가
                        description_str = description if description else None
                        actions.append(f'[설명] : {to_str(keyinfo_data.description)} → {description_str}')

                        # 항목 수정
                        models.Keyinfo.objects.filter(user_id=request.user.id, key=key).update(description=description)
                        ...
                        if resp_body and resp_body.get('user_data'):
                            ...
                            if check_password(user_data, request.user.password):
                                ...
                                if models.Keyinfo.objects.filter(user_id=request.user.id, key=key):
                                    ...
                                    # 키 삭제 수행
                                    models.Keyinfo.objects.filter(user_id=request.user.id, key=key).delete()
            ...
            if key:
                ...
                if req_pw == 'Y':
                    ...
                    if user_data:
                        ...
                        if check_password(user_data, request.user.password):
                            if models.Keyinfo.objects.filter(user_id=request.user.id, key=key):
                                value = models.Keyinfo.objects.get(user_id=request.user.id, key=key).value
...
class AdminAPI(View):
    ...
    def get(self, request):
        ...
        try:
            # 컬럼 리스트
            column_list = ('user_id', 'user_id__username', 'key', 'value', 'description', 'created_date')
            ...
            data_cols_args = ['user_id', 'user_id__username', 'key', 'value', 'description']
            data_cols_kwargs = dict(
                date=Cast(TruncSecond('created_date', DateTimeField()), CharField())
            )
            val = list(models.Keyinfo.objects.values(
    ...
    def post(self, request):
        ...
        try:
            ...
            insert_audit_log(request.user.id, request, category, '관리자', audit_log, result)
            ...
            if key:
                ...
                if User.objects.filter(pk=to_int(user)):
                    ...
                    if models.Keyinfo.objects.filter(user_id=user_obj.id, key=key):
                        keyinfo_data = models.Keyinfo.objects.get(user_id=user_obj.id, key=key)
                        ...
                        if keyinfo_data.description != description:
                            ...
                            # 항목 수정
                            models.Keyinfo.objects.filter(user_id=user_obj.id, key=key).update(description=description)
    ...
    def delete(self, request, key):
        ...
        try:
            if key and request.body and json.loads(request.body) and json.loads(request.body).get('user_id'):
                resp_body = json.loads(request.body)
                        ...
                        if resp_body.get('user_data'):
                            ...
                            if check_password(user_data, request.user.password):
                                ...
                                if models.Keyinfo.objects.filter(user_id=user_obj.id, key=key):
                                    ...
                                    # 키 삭제 수행
                                    models.Keyinfo.objects.filter(user_id=user_obj.id, key=key).delete()
            ...
            if User.objects.filter(pk=to_int(user_id)) and key:
                ...
                if req_pw == 'Y':
                    ...
                    if user_data:
                        ...
                        if check_password(user_data, request.user.password):
                            if models.Keyinfo.objects.filter(user_id=user_obj.id, key=key):
                                value = models.Keyinfo.objects.get(user_id=user_obj.id, key=key).value
                                dec_value = get_dec_value(value)
                                result = True
...
class KeyInfoSettingsAPI(View):
    ...
    def get(self, request):
        try:
            ...
            return JsonResponse(
                {'draw': draw, 'recordsTotal': len(val), 'recordsFiltered': len(val), 'data': val})
    ...
    def put(self, request):
        ...
        try:
            ...
            # 감사 로그 기록
            audit_log = f"""편집 ( {', '.join(actions)} )"""
            insert_audit_log(request.user.id, request, '시스템 설정', '시스템 설정', audit_log, result)
