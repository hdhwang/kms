...
class AuditLogAPI(View):
    ...
    def get(self, request):
        try:
            # 컬럼 리스트
            column_list = ('user__username', 'ip', 'category', 'sub_category', 'action', 'result', 'date')
            ...
            for param in request.GET.items():
                ...
                # 필터 설정
                    ...
                    # 사용자
                    if filter_name == 'user__username':
                        insert_dic_data(filter_params, 'user__username__icontains', value)
            ...
            data_cols_args = ['user__username', 'ip', 'category', 'sub_category', 'action', 'result']
            data_cols_kwargs = dict(
                audit_date=Cast(TruncSecond('date', DateTimeField()), CharField())
            )
            val = list(models.AuditLog.objects.extra(select={'ip': 'inet_ntoa(ip)'}).values(
