...
def api(request):
    try:
            ...
            if check_settings(request.method):
                if request.GET:
                    if len(request.GET.getlist("key")) > 1:
                        actions.append(
                            f'[요청]: {list_to_str(request.GET.getlist("key"))} (KEY 파라미터 다중 사용)'
                        )
                        return JsonResponse(
                            status=400,
                            data={"detail": "Only one key parameter can be used"},
                        )

                    if request.GET.get("key"):
                        key_list = request.GET.get("key").replace(" ", "").split(",")
                        actions.append(f"[요청]: {list_to_str(key_list)}")
                        queryset = models.Keyinfo.objects.filter(
                            user_id=userID, key__in=key_list
                        )
                ...
                actions.append(
                    f"[응답]: {list_to_str(exists_key_list)}"
                    if exists_key_list
                    else f", [응답]: None"
                )
                result = True
        ...
        # 키 저장
            ...
            if check_settings(request.method):
                ...
                if chk_json is False:
                    ...
                    return JsonResponse(
                        status=400,
                        data={"detail": "Invalid syntax or Missing required value"},
                    )

                # body 내 중복 키 존재 유무 체크
                if len(key_list) != len(set(key_list)):
                    actions.append(
                        f"[요청]: {list_to_str(key_list)} (요청 내용에 중복된 KEY가 존재)"
                    )
                    return JsonResponse(
                        status=400,
                        data={"detail": "Duplicate key exists in requested value"},
                    )

                # 사용할 수 없는 문자 존재 유무 체크
                if character_validator(key_list):
                    actions.append(
                        f"[요청]: {list_to_str(key_list)} (KEY에 사용할 수 없는 문자가 포함되어 있음)"
                    )
                    return JsonResponse(
                        status=422,
                        data={"detail": "Contains characters that cannot be used"},
                    )
                ...
                if models.Keyinfo.objects.filter(
                    ...
                    data = models.Keyinfo.objects.filter(
                        user_id=userID, key__in=key_list
                    )
                    exists_key_list = []
                    for i in data:
                    ...
                    actions.append(
                        f"[요청]: {list_to_str(key_list)} (요청 내용에 이미 등록되어 있는 KEY 존재)"
                    )
                    return JsonResponse(
                        status=400,
                        data={"detail": f"Key already registered {exists_key_list}"},
                    )
            ...
            if check_settings(request.method):
                if request.GET and len(request.GET.getlist("key")):
                    ...
                    if len(key_list) > 1:
                        ...
                        return JsonResponse(
                            status=400, data={"detail": "Only one key can be entered"}
                        )
                    ...
                    if len(request.data["data"]) > 1:
                        ...
                        return JsonResponse(
                            status=400,
                            data={"detail": "Only one description can be entered"},
                        )

                    if request.GET.get("key"):
                        # key가 존재하면 수정 수행
                        if models.Keyinfo.objects.filter(
                            user_id=userID, key=request.GET.get("key")
                        ).exists():
                            data = models.Keyinfo.objects.get(
                                user_id=userID, key=request.GET.get("key")
                            )
                            ...
                            actions.append(
                                f'[요청]: (KEY:"{request.GET.get("key")}"){list_to_str(request.data["data"])}'
                            )
                            actions.append(
                                f'[처리]: {org_desc} → {request.data["data"][0]["description"]}'
                            )
                            result = True
                            return HttpResponse(status=204)
                        else:
                            ...
                            return JsonResponse(
                                status=400, data={"detail": "No key to modify"}
                            )

                actions.append(f"필수 파라미터 누락")
                return JsonResponse(
                    status=400, data={"detail": "Missing Required Parameter"}
                )
        ...
        # 키 삭제 (단일 키만 삭제 가능)
            ...
            if check_settings(request.method):
                if request.GET and len(request.GET.getlist("key")):
                    ...
                    if len(key_list) > 1:
                        actions.append(
                            f'[요청]: {to_str(request.GET.get("key"))} (KEY 다중 입력)'
                        )
                        return JsonResponse(
                            status=400, data={"detail": "Only one key can be entered"}
                        )
                    ...
                    if request.GET.get("key"):
                        ...
                        if models.Keyinfo.objects.filter(
                            ...
                            return JsonResponse(
                                status=400, data={"detail": "No key to delete"}
                            )
        ...
        insert_audit_log(
            request.user.username, request, category, sub_category, audit_log, result
        )
...
def check_settings(method):
    try:
        key_dic = dict(
            GET="ENABLE_GET_API",
            POST="ENABLE_POST_API",
            PUT="ENABLE_PUT_API",
            DELETE="ENABLE_DELETE_API",
        )
...
def json_validator(body_data, method):
    try:
        ...
        if method == "PUT":
            keyinfo_schema = {
                "properties": {"description": {"type": "string", "maxLength": 128}},
                "additionalProperties": False,
