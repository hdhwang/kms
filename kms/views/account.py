...
class UsersAPI(View):
    ...
    def get(self, request):
        try:
            ...
            data_cols_kwargs = dict(
                _last_login=Cast(
                    TruncSecond("last_login", DateTimeField()), CharField()
                ),
                _date_joined=Cast(
                    TruncSecond("date_joined", DateTimeField()), CharField()
                ),
            )
            val = list(
                User.objects.values(*data_cols_args, **data_cols_kwargs)
    ...
    def post(self, request):
        ...
        try:
            ...
            insert_audit_log(
                request.user.username, request, category, "사용자 관리", audit_log, result
            )
    ...
    def put(self, request, req_id):
        ...
        try:
            if req_id:
                ...
                if (
                    user_id != to_int(req_id)
                    and to_str(data.is_superuser) != is_superuser
                ):
...
class TokenView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.context["tokenless_user_list"] = list(
            User.objects.values(*data_cols_args)
            .filter(**filter_params)
            .order_by("username")
        )
...
class TokenAPI(View):
    ...
    def get(self, request):
        try:
            ...
            val = list(
                Token.objects.values(
                    "user_id__username",
                    "key",
                    created_date=Cast(
                        TruncSecond("created", DateTimeField()), CharField()
                    ),
                )
                .filter(**filter_params)
                .order_by(ordering)[limit_start:limit_end]
    ...
    def post(self, request):
        ...
        try:
            ...
            insert_audit_log(
                request.user.username, request, category, "토큰 관리", audit_log, result
            )
    ...
    def delete(self, request, req_key):
        ...
        try:
            ...
            if (
                ...
                and json.loads(request.body).get("user_data")
            ):
