...
class IPAddrAPI(View):
    ...
    def get(self, request):
        try:
            ...
            val = list(
                models.IpWhitelist.objects.extra(select={"ip": "inet_ntoa(ip)"})
                .values(*data_cols_args)
        ...
        try:
            ...
            # 동일한 IP 주소가 존재하지 않는 경우
                ...
                ip_whitelist = models.IpWhitelist(
                    ip=ip_to_int(ip), cidr=cidr, comment=comment
                )
                ip_whitelist.save()
                result = True
            ...
            insert_audit_log(
                request.user.username, request, category, "IP 접근 제어", audit_log, result
            )

    @method_decorator(
        permission_required("kms.change_ipwhitelist", raise_exception=True)
    )
    ...
    @method_decorator(
        permission_required("kms.delete_ipwhitelist", raise_exception=True)
    )
