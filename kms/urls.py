...
urlpatterns = [
    ...
    # 비밀번호 변경
    path('account/', login_required(auth_view.PasswordChangeView.as_view(template_name='kms/account/account.html')), name='account'),
    ...
    # 계정 관리 > 토큰 관리
    path('account/token', login_required(account.TokenView.as_view()), name='account_token'),
    ...
    # 시스템 설정
    path('keyinfo/settings', login_required(keyinfo.KeyInfoSettingView.as_view()), name='keyinfo_settings'),
