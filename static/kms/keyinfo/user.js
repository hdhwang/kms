...
const requestUrl = '/keyinfo/user/api';   //API URL
const requestCheckUserIDUrl = '/keyinfo/user/check'; //키 중복 확인 URL
const requestValueUrl = '/keyinfo/user/value';   //키 복호화 API URL
...
function initDataTable() {
    ...
    let table = $('#table-data').DataTable({
        ...
        columns: [                              //출력할 컬럼 정보
            ...
            { className: 'dt-center link', data: 'key', render: $.fn.dataTable.render.text() },
            {
                className: 'dt-center',
                data: null,
                defaultContent: '',
        ...
        order: [[5, 'desc']],                                  //조회 시 생성 일자 기준 기본 정렬
    ...
    axios(
        ...
        .then(function (response) {
            ...
            if (response && response.data && response.data.data) {
                openValueModal(key, response.data.data);
            }
...
function openModal(data = null) {
    ...
    $('#btn-ok').off('click').on('click', function () {
        if ($.trim($('#keyinfo-id').val()) === '') {
            showModal('경고', '키를 입력하십시오.');
        }

        else if (checkedKeyInfoID == false || (checkedKeyInfoIDText != $('#keyinfo-id').val())) {
...
function openValueModal(key, value) {
    ...
    $('#menu-value-modal').off('show.bs.modal').on('show.bs.modal', function () {
        ...
        $('#modal-value-title').text(key); //타이틀을 키로 변경
...
function deleteData(key, data = null) {
    axios(
        {
            method: 'DELETE',
            url: requestUrl + '/' + key,
